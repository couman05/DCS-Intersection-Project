...
public class Activation implements Serializable {
	...
	private void Square() throws CloneNotSupportedException {
		...
		if (inputIndex == -1) {
			// Input place not found in temporary marking, check in constant places
			inputIndex = util.GetIndexByName(InputPlaceName, Parent.Parent.ConstantPlaceList);
		}

		if (inputIndex == -1) {
			// Input place not found, cannot calculate square
			return;
		}
		...
		if (temp instanceof DataFloat) {
			float inputValue = (float) temp.GetValue();
			float resultValue = inputValue * inputValue; // Calculate the square
			DataFloat result = new DataFloat();
